...
RUN apk --no-cache update && apk --no-cache upgrade && apk --no-cache add --virtual buildpack gcc musl-dev build-base binutils libc-dev && apk --no-cache add git postgresql-dev zlib-dev jpeg-dev tzdata
